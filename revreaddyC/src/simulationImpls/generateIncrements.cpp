...
namespace janek {
        ...
        double factor = sqrt(D * std::pow(tau, alpha)* std::pow((double) N, alpha) / (N));

        for (int ii = 0; ii < Nextended; ++ii) {
            if (ii <= N) {
                in[ii] =  (1 - std::pow((double) ii / N, alpha)) / 2;
            }
    ...
    boost::multi_array<double, 2> generateIncrementsdaivs(unsigned long N, double &D, double &tau, double &alpha, Random *random) {
        //boost::multi_array<double, 2> increments;
        int dimensions = 3;
        auto Nextended = N * 2;
        typedef boost::multi_array<double, 2> array_type;
