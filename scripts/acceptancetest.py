...
accname = "data/acc_" + time.strftime("%Y_%m_%d-%H_%M_%S") + ".dat"
energyname = "data/energy_" + time.strftime("%Y_%m_%d-%H_%M_%S") + ".dat"
simulation.new_Acceptance(1, accname)
simulation.new_Energy(1, energyname)
...
acc = np.transpose(np.loadtxt(accname, skiprows=1))
energy = np.transpose(np.loadtxt(energyname, skiprows=1))
